 ...
 public:
    ...
    get_cells(const std::vector<cid_t>& cids) override {
        ...
        for (cid_t cid : cids) {
            ...
            result.emplace_back(cid, std::make_unique<TestCell>(static_cast<int>(cid * 10), cid, estimated_byte_size_of_cell(cid).first));
                ...
                for (cid_t extra_cid : extra_cids->second) {
                    ...
                    if (std::find(cids.begin(), cids.end(), extra_cid) == cids.end() &&
                        ...
                        result.emplace_back(extra_cid,
                                            std::make_unique<TestCell>(static_cast<int>(extra_cid * 10), extra_cid, estimated_byte_size_of_cell(extra_cid).first));
                    }
                }
            }
        }
