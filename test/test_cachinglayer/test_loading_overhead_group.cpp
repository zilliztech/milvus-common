...
class LoadingOverheadGroupTest : public ::testing::Test {
 protected:
    ...
    BindMemory(const LoadingOverheadGroupHandle& group, std::optional<int64_t> max_runtime_unit = std::nullopt) {
...
TEST_F(LoadingOverheadGroupTest, ZeroBudgetDoesNotCapReservation) {
    auto group = CreateMemoryGroup(LoadingOverheadPolicy::Budget(0));
    auto config = BindMemory(group, 50);

    EXPECT_EQ(Reserve(config, {500, 0}), (ResourceUsage{500, 0}));
    EXPECT_EQ(Release(config, {500, 0}), (ResourceUsage{500, 0}));
}

TEST_F(LoadingOverheadGroupTest, ExecutorBoundSaturatesOnOverflow) {
    auto group = CreateMemoryGroup(LoadingOverheadPolicy::Executor(std::numeric_limits<int64_t>::max()));
    auto config = BindMemory(group, 2);
