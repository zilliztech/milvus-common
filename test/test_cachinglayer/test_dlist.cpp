...
TEST_F(DListTest, ReserveWithCancellationTokenImmediateSuccess) {
    ...
    ResourceUsage size{20, 10};
    auto future = dlist->ReserveLoadingResourceWithTimeout(size, std::chrono::milliseconds(1000), &cancel_token);
...
TEST_F(DListTest, ReserveWithCancellationTokenCancelledWhileWaiting) {
    ...
    auto future =
        dlist->ReserveLoadingResourceWithTimeout(reserve_size, std::chrono::milliseconds(5000), &cancel_token);
...
TEST_F(DListTest, ReserveWithCancellationTokenMultipleRequests) {
    ...
    ResourceUsage reserve_size{50, 30};
    auto future1 =
        dlist->ReserveLoadingResourceWithTimeout(reserve_size, std::chrono::milliseconds(5000), &cancel_token1);
    auto future2 =
        dlist->ReserveLoadingResourceWithTimeout(reserve_size, std::chrono::milliseconds(5000), &cancel_token2);
...
TEST_F(DListTest, ReserveWithNullCancellationToken) {
    ...
    ResourceUsage size{20, 10};
    auto future = dlist->ReserveLoadingResourceWithTimeout(size, std::chrono::milliseconds(1000), nullptr);
    EXPECT_TRUE(std::move(future).get());
    EXPECT_EQ(get_loading_memory(), size);
}
...
TEST_F(DListTest, ReserveWithCancellationTokenTimeoutBeforeCancel) {
    ...
    // Very short timeout
    ResourceUsage reserve_size{50, 30};
    auto future = dlist->ReserveLoadingResourceWithTimeout(reserve_size, std::chrono::milliseconds(50), &cancel_token);
...
TEST_F(DListTest, ReserveWithCancellationTokenSucceedsWhenResourcesFreed) {
    ...
    ResourceUsage reserve_size{60, 30};
    auto future =
        dlist->ReserveLoadingResourceWithTimeout(reserve_size, std::chrono::milliseconds(1000), &cancel_token);
