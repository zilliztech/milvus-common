...
class sherwood_v10_table : private EntryAlloc, private Hasher, private Equal, private BucketAllocator {
 ...
 public:
    ...
    explicit sherwood_v10_table(const ArgumentAlloc& alloc) : EntryAlloc(alloc), BucketAllocator(alloc) {
    }
    ...
    erase(const_iterator begin_it, const_iterator end_it) {
        ...
        while (*previous != begin_it.current_element) previous = &(*previous)->next;
 ...
 private:
    ...
    void
    swap_pointers(sherwood_v10_table& other) {
        using std::swap;
...
class unordered_set : public detailv10::sherwood_v10_table<
 ...
 public:
    ...
    friend bool
    operator==(const unordered_set& lhs, const unordered_set& rhs) {
        if (lhs.size() != rhs.size())
            return false;
