...
struct TrackingStdAllocator : public std::allocator<T> {
    ...
    void
    reset_counter() noexcept {
        counter_->store(0, std::memory_order_relaxed);
    }

    template <class U>
    bool
    operator==(const TrackingStdAllocator<U>& other) const noexcept {
 ...
 private:
    std::shared_ptr<std::atomic<size_t>> counter_{std::make_shared<std::atomic<size_t>>(0)};
};
