...
struct AutoSpan {
    ...
    // keep the disabled path allocation-free. (On master they forward to the const char*
    // StartSpan() overloads from #103; that change is not on 2.6, so when tracing is *enabled* the
    // std::string is still built inside StartSpan - which is fine, an enabled span costs far more
    // than one small allocation.)
