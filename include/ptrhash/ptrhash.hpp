...
inline uint64_t
mul_high(uint64_t a, uint64_t b) {
...
inline size_t
fast_reduce(uint64_t d, uint64_t h) {
    return static_cast<size_t>(mul_high(d, h));
}

inline uint64_t
fastmod32_multiplier(size_t d) {
    return (std::numeric_limits<uint64_t>::max() / static_cast<uint64_t>(d)) + 1;
}

inline size_t
fastmod32_reduce(uint64_t d, uint64_t m, uint64_t h) {
...
inline uint64_t
hash_key(uint64_t key, uint64_t seed) {
#if defined(__SIZEOF_INT128__)
...
inline uint64_t
read_u64_unaligned(const uint8_t* p) {
    uint64_t value = 0;
...
inline uint64_t
hash_bytes(std::string_view key, uint64_t seed) {
...
inline uint64_t
hash_key_for(std::string_view key, uint64_t seed) {
    return hash_bytes(key, seed);
}

inline uint64_t
hash_pilot(uint64_t pilot, uint64_t seed) {
...
inline uint64_t
bucket_transform(uint64_t x, BucketFunction bucket_function) {
...
inline void
append_u32(std::vector<uint8_t>& out, uint32_t value) {
    for (int i = 0; i < 4; ++i) {
...
inline void
append_u64(std::vector<uint8_t>& out, uint64_t value) {
    for (int i = 0; i < 8; ++i) {
        out.push_back(static_cast<uint8_t>(value >> (8 * i)));
    }
}
...
inline size_t
choose_parts(size_t n, double alpha) {
    if (n == 0) {
        return 0;
    }
...
class PtrHashView {
 public:
    ...
    static PtrHashView
    from_bytes(const void* data, size_t size) {
 ...
 private:
    ...
    size_t
    index_no_remap_hx(uint64_t hx) const {
        if (n_ == 0) {
    ...
    size_t
    index_from_slot(size_t slot) const {
...
class PtrHash {
 public:
    ...
    PtrHash&
    operator=(PtrHash&& other) noexcept {
        if (this != &other) {
    ...
    static PtrHash
    build(const std::vector<Key>& keys, const PtrHashParams& params = PtrHashParams()) {
        return build_impl(keys, params, detail::KeyHashKind::Integer);
    }

    static PtrHash
    build(const std::vector<std::string>& keys, const PtrHashParams& params = PtrHashParams()) {
        return build_impl(keys, params, detail::KeyHashKind::String);
    }

    static PtrHash
    build(const std::vector<std::string_view>& keys, const PtrHashParams& params = PtrHashParams()) {
        return build_impl(keys, params, detail::KeyHashKind::String);
    }

    static PtrHash
    build_hashes(const std::vector<uint64_t>& hashes, const PtrHashParams& params = PtrHashParams()) {
 ...
 private:
    ...
    build_impl(const std::vector<Key>& keys, const PtrHashParams& params, detail::KeyHashKind key_hash_kind) {
        ...
        for (size_t attempt = 0; attempt < params.max_seed_attempts; ++attempt) {
            ...
            if (try_build(keys, parts, params.alpha, params.lambda, seed, params.max_pilot, params.build_threads,
                          params.bucket_function, slots_total, buckets_total, slots_per_part, buckets_per_part, pilots,
                          remap)) {
                return from_parts(n, slots_total, buckets_total, parts, slots_per_part, buckets_per_part, seed,
                                  params.bucket_function, key_hash_kind, std::move(pilots), std::move(remap));
            }
        }
 ...
 public:
    static PtrHash
    deserialize(const void* data, size_t size) {
 ...
 private:
    ...
    template <typename Key>
    static void
    validate_unique(const std::vector<Key>& keys) {
    ...
    template <typename Key>
    static bool
    try_build(const std::vector<Key>& keys, size_t parts, double alpha, double lambda, uint64_t seed,
              uint16_t max_pilot, size_t build_threads, BucketFunction bucket_function, size_t& slots_total,
        ...
        fill_buckets(keys, parts, buckets_per_part, buckets_total, seed, bucket_function, build_threads, bucket_starts,
                     bucket_hashes);
    ...
    static size_t
    effective_thread_count(size_t requested, size_t limit) {
    ...
    static size_t
    build_thread_count(size_t n, size_t requested) {
        return effective_thread_count(requested, (n + 99999) / 100000);
    }

    static size_t
    bucket_for_hash(uint64_t hx, size_t parts, size_t buckets_per_part, size_t buckets_total,
    ...
    template <typename Key>
    static void
    fill_buckets(const std::vector<Key>& keys, size_t parts, size_t buckets_per_part, size_t buckets_total,
                 uint64_t seed, BucketFunction bucket_function, size_t build_threads,
                 std::vector<uint32_t>& bucket_starts, std::vector<uint64_t>& bucket_hashes) {
    ...
    static size_t
    slot_in_part_hp(uint64_t hx, uint64_t rem_slots_m, uint64_t pilot_hash, size_t slots_per_part) {
    ...
    static bool
    bucket_slots(const std::vector<uint64_t>& hashes, size_t begin, size_t end, uint64_t rem_slots_m,
                 uint64_t pilot_hash, size_t slots_per_part, std::vector<size_t>& out) {
        out.clear();
    ...
    static bool
    bucket_slots_available(const std::vector<uint64_t>& hashes, size_t begin, size_t end, uint64_t rem_slots_m,
                           uint64_t pilot_hash, size_t slots_per_part, const uint8_t* taken_part) {
        size_t i = begin;
    ...
    static bool
    try_take_bucket_slots(const std::vector<uint64_t>& hashes, size_t begin, size_t end, uint64_t rem_slots_m,
                          uint64_t pilot_hash, size_t slots_per_part, uint8_t* taken_part, std::vector<size_t>& out) {
        out.clear();
        out.reserve(end - begin);
        for (size_t i = begin; i < end; ++i) {
            const size_t slot = slot_in_part_hp(hashes[i], rem_slots_m, pilot_hash, slots_per_part);
    ...
    static bool
    contains_recent(const std::array<BucketId, 16>& recent, BucketId bucket) {
        return std::find(recent.begin(), recent.end(), bucket) != recent.end();
    }

    static bool
    build_part(size_t part, size_t buckets_per_part, size_t slots_per_part, uint64_t rem_slots_m, uint16_t max_pilot,
               const std::array<uint64_t, 256>& pilot_hashes, const std::vector<uint64_t>& bucket_hashes,
               const std::vector<uint32_t>& bucket_starts, std::vector<uint8_t>& pilots, std::vector<uint8_t>& taken) {
        ...
        for (BucketId new_bucket : order) {
            ...
            while (!stack.empty()) {
                ...
                for (uint32_t pilot_u32 = 0; pilot_u32 <= max_pilot; ++pilot_u32) {
                    ...
                    if (!bucket_slots_available(bucket_hashes, begin, end, rem_slots_m, pilot_hash, slots_per_part,
                                                taken_part)) {
                        continue;
                    }
                    if (!try_take_bucket_slots(bucket_hashes, begin, end, rem_slots_m, pilot_hash, slots_per_part,
                                               taken_part, candidate_slots)) {
                        continue;
                    }
                ...
                for (uint32_t pilot_u32 = 0; pilot_u32 <= max_pilot; ++pilot_u32) {
                    const auto pilot = static_cast<uint16_t>(pilot_u32);
                    const uint64_t pilot_hash = pilot_hashes[pilot_u32];
                    if (!bucket_slots(bucket_hashes, begin, end, rem_slots_m, pilot_hash, slots_per_part,
                                      candidate_slots)) {
                        continue;
                    }
                    size_t score = 0;
                ...
                if (!bucket_slots(bucket_hashes, begin, end, rem_slots_m, pilot_hashes[best_pilot], slots_per_part,
                                  candidate_slots)) {
                    return false;
                }
                ...
                for (size_t slot : candidate_slots) {
                    ...
                    if (other != bucket_npos() && other != bucket) {
                        ...
                        if (!bucket_slots(bucket_hashes, other_begin, other_end, rem_slots_m, pilot_hashes[other_pilot],
                                          slots_per_part, remove_slots)) {
                            return false;
                        }
    ...
    static constexpr BucketId
    bucket_npos() {
        return std::numeric_limits<BucketId>::max();
    }

    static PtrHash
    from_parts(size_t n, size_t slots_total, size_t buckets_total, size_t parts, size_t slots_per_part,
...
class PtrHashWithHasher {
 public:
    ...
    static PtrHashWithHasher
    deserialize(const void* data, size_t size, Hasher hasher = Hasher()) {
        return PtrHashWithHasher(PtrHash::deserialize(data, size), std::move(hasher));
    }

    static PtrHashWithHasher
    load(const std::string& path, Hasher hasher = Hasher()) {
    ...
    size_t
    index_no_remap(const Key& key) const {
        return hash_.index_no_remap_hash(hash_key(key));
    }
    size_t
    index(const Key& key) const {
 ...
 private:
    ...
    static uint64_t
    to_u64_hash(Value value) {
        using Decayed = typename std::decay<Value>::type;
        static_assert(std::is_integral<Decayed>::value && sizeof(Decayed) <= sizeof(uint64_t),
...
class MappedPtrHash {
 public:
    ...
    MappedPtrHash&
    operator=(const MappedPtrHash&) = delete;
    ...
    MappedPtrHash&
    operator=(MappedPtrHash&& other) noexcept {
        if (this != &other) {
            close();
            move_from(std::move(other));
        }
    ...
    static MappedPtrHash
    open(const std::string& path, size_t offset = 0) {
#if defined(__unix__) || defined(__APPLE__)
 ...
 private:
    ...
    void
    move_from(MappedPtrHash&& other) noexcept {
